...
class Exemplo03FormularioControle extends TPage
{
    protected $form; 
    private $formTitle      = 'Exemplo03FormularioControle';
    ...
    public function __construct( $param=null, $embbed=false )
    {
        ...
        $campo_uppercase = MField::fieldUppercase('descricao');
        $campo_arquivo = MField::TMFile('arquivo');
        $campo_mes = MField::selectMes('mes');
        $campo_valor = MField::fieldValor('valor');
        $campo_data = MField::fieldDate('data');
        $campo_inteiro = MField::fieldInteger('integer','9999');
        $campo_telefone = MField::fieldTelefone('telefone');
        $campo_editor = MField::fieldHtmlEditor('editor');
        $campo_memo   = MField::fieldText('campo_memo',100,100);

        // $anobase                  = MField::fieldInteger('anobase');
        // $anobase->setValue(MSispubGlobal::getGlobalAnobase());
        // $anobase->setEditable(FALSE);
        
        $this->form->addFields( [ new TLabel('Descricao em Maiusculo') ], [ $campo_uppercase ] );        
        $this->form->addFields( [ new TLabel('arquivo')   ], [ $campo_arquivo ] );
        $this->form->addFields( [ new TLabel('Mes')       ], [ $campo_mes ] );
        $this->form->addFields( [ new TLabel('Valor')     ], [ $campo_valor ] );
        $this->form->addFields( [ new TLabel('data')      ], [ $campo_data ] );
        $this->form->addFields( [ new TLabel('Inteiro')   ], [ $campo_inteiro ] );
        $this->form->addFields( [ new TLabel('Telefone')   ], [ $campo_telefone ] );
        $this->form->addFields( [ new TLabel('Memo')   ], [ $campo_memo ] );

        $this->form->addFields( [ new TLabel('Editor')   ], [ $campo_editor ] );


        //   validacoes
        MField::validarCampo($campo_uppercase   ,'Campo em Maiusculo'  , ['req'=>'', 'minlen'=>'4']); // obrigatorio. Tamanho minimo de 4
        MField::validarCampo($campo_arquivo   ,'Arquivo'  , ['req'=>'']);
        MField::validarCampo($campo_mes   ,'Mes Referencia' , ['req'=>'']);
        MField::validarCampo($campo_valor   ,'Valor'        , ['req'=>'']);
        MField::validarCampo($campo_data   ,'Data'    , ['req'=>'']);
        MField::validarCampo($campo_inteiro   ,'Valor Inteiro'   , ['req'=>'']);
